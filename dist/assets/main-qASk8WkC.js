...
contentNodes.forEach((node) => {
  gsapWithCSS.set(node, { display: "none", flexDirection: "column" });
});
gsapWithCSS.set(currentStepContent, { display: "flex" });
...
const tlForwardCardAnimation = (index) => {
  ...
  tl.to(steps[index].querySelector(".lw-calc__step-underlay"), { x: 0, duration: 0.4, ease: "power2.out" });
  tl.to(steps[index].querySelector(".lw-calc__step-value span"), { color: "#ffffff", duration: 0.4, ease: "power2.out" }, "-=0.3");
  tl.to(steps[index].querySelector(".lw-calc__step-value small"), { visibility: "visible", height: "auto", autoAlpha: 1, color: "#ffffff", duration: 0.5, ease: "power2.out" }, "-=0.3");
...
const tlBackCardAnimation = (index) => {
  ...
  tl.to(lastViewedStep, { autoAlpha: 0.1, duration: 0.4 });
  tl.fromTo(contentNodes[index - 1], { x: "20px", autoAlpha: 0, display: "none" }, {
    display: "flex",
    x: 0,
    autoAlpha: 1,
...
const onControlChangeRefreshStateHandler = (ctrl) => {
  ...
  for (let i = index + 1; i < contentNodes.length; i++) {
    ...
    fields.forEach((field) => {
      ...
      if (ctrls.length) {
        ...
        const textureCtrls2 = field.querySelectorAll("[data-texture-collection-id]");
        if (textureCtrls2) {
  ...
  const tl = gsapWithCSS.timeline();
  tl.to(cards, { stagger: 0.1, className: "lw-calc__step", autoAlpha: 0.1, duration: 0.4 }).to(underlay, { stagger: 0.1, x: "-160px", duration: 0.4 }, "-=.3").to(title, { stagger: 0.1, color: "#000000", duration: 0.4 }, "-=.9").to(text, {
    duration: 0.4,
    opacity: 0,
    height: 0,
    autoAlpha: 0,
...
if (collectionSwitchers) {
  collectionSwitchers.forEach((switcher) => {
    switcher.addEventListener("change", () => {
      ...
      const textureCtrls2 = switcher.closest(".lw-steps-content").querySelectorAll("[data-texture-collection-id]");
      const arr = [];
      textureCtrls2.forEach((ctrl) => {
...
const disableTextureCollections = (disabler, state) => {
  const textureCtrls2 = disabler.closest(".lw-steps-content").querySelectorAll("[data-texture-collection-id]");
